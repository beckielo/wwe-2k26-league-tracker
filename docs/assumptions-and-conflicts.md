...
- **Current handling:** the import maps only the decisive winner/loser rows demonstrated by the workbook. The manual Result Entry page permits schedule-locked decisive-result validation only and performs no workbook write. Draw, DQ, no-contest, and unclear-result Excel export remain disabled until their workbook encoding is documented. Phase 2B may show Draw or No Contest only as an editable browser-local simulation preview under C-012; those outcomes are never mapped back into Excel.
...
- **Persistence:** Phase 2B originally stored a simulation preview in browser state. Phase 3A promotes confirmed previews into the shared versioned tracker-state overlay under C-013. The workbook and server snapshot remain unchanged.
...
## Phase 3C — workflow presentation

- **Shared recommendation:** Dashboard, Result Entry, Simulation, and Week Review use one pure workflow summary derived from the workbook completed-through week, authoritative scheduled matches, browser-local confirmed results, local completed-week locks, and the workbook-defined user league.
- **Recommendation order:** missing user-league results point to Result Entry; after the user show is complete, missing non-user results point to Simulation; once all 24 results are valid, the recommendation points to Week Review for locking; when no authoritative later week remains, the workflow reports completion.
- **Presentation-only scope:** Phase 3C does not change scoring, result validation, workbook parsing, tracker-state version 1, JSON compatibility, or Excel write behavior.
- **Per-league simulation progress:** non-user league counts are calculated only from authoritative active-week schedule rows and valid browser-local confirmations. They do not create or infer fixtures.

## Phase 4 — weekly close exports

- Weekly close JSON and CSV files are generated from the read-only workbook baseline plus browser-local confirmed results and completed-week locks.
- A clean weekly close package is exportable only for a locally locked week with exactly 24 authoritative scheduled matches and 24 valid confirmed results.
- Weekly result exports retain result source, result type, winner where applicable, and No Contest confirmations. No Contest continues to have no standings effect.
- Exporting files does not update tracker-state version 1 and does not modify, overwrite, or write back to the Excel workbook.
