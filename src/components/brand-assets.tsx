...
export function LeagueBrandMark({ league, usage = "secondary", className = "" }: { league: LeagueName; usage?: BrandUsage; className?: string }) {
  ...
  return <span className={`league-brand-mark brand-${usage} league-${asset.id} ${className}`} style={style} data-brand-fallback={asset.fallbackCrest}>
    <ResilientBrandImage src={asset.assetPath} alt={`${league} custom league logo`} className="brand-image" />
...
export function LeagueWatermark({ league }: { league: LeagueName }) {
  return <LeagueBrandMark league={league} usage="ambient" className="league-watermark" />;
}
