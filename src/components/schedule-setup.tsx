...
export function ScheduleSetupView(props: Props) {
  ...
  const accept = () => {
    ...
    setMessage({ tone: "success", text: "Schedule snapshot accepted. Closing Split Week 1 activation is now available." });
  };
  ...
  return <div className="space-y-8">
    ...
    {targetCompositionAccepted && <section className="border border-emerald-400/30 bg-emerald-400/10 p-6"><h2 className="text-xl font-black uppercase">Accepted post-finals roster baseline</h2><p className="mt-2 text-sm text-emerald-50">Schedule setup is using the reviewed browser-local composition for League Year {targetYear} · {targetSplit}. No fixtures are created until Generate or Import is clicked.</p><dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4"><Metadata label="acceptedAt" value={acceptedComposition?.acceptedAt ?? ""} /><Metadata label="sourceSignature" value={acceptedComposition?.sourceSignature ?? ""} /><Metadata label="target" value={`League Year ${targetYear} · ${targetSplit}`} /><Metadata label="rosterCount" value={String(LEAGUE_NAMES.flatMap((league) => acceptedComposition?.rosters[league] ?? []).length)} /></dl></section>}
