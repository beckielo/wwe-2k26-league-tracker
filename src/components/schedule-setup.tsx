...
export function ScheduleSetupView(props: Props) {
  ...
  return <div className="space-y-8">
    <section className="grid gap-4 md:grid-cols-3">{[["Post-Finals transition", transitionValid], ["Seed continuity", readiness.seeds.valid], [targetSplit === "Closing Split" ? "Closing Split Week 1" : "Opening Split Week 1", unlocked]].map(([label, valid]) => <div key={String(label)} className={`border p-5 ${valid ? "border-emerald-400/30 bg-emerald-400/10" : "border-amber-400/30 bg-amber-400/10"}`}><p className="text-xs font-black uppercase">{label}</p><strong className="mt-2 block">{valid ? "Ready" : "Locked"}</strong></div>)}</section>
