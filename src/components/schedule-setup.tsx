...
export function ScheduleSetupView(props: Props) {
  ...
  if (closingRegularSeasonComplete) {
    return <div className="space-y-8">
      <section className="border border-emerald-400/30 bg-emerald-400/10 p-6">
        ...
        {needsPostRegularRecovery && <p className="mt-2 text-sm font-bold text-amber-100">Recovered saved progress from Closing Split Week 22 / Year Week 46 without resetting results or locked weeks.</p>}
      ...
      {state.acceptedSchedule && <details className="border border-emerald-400/30 bg-[#111722] p-6"><summary className="cursor-pointer text-sm font-black uppercase text-emerald-200">Accepted schedule details</summary><p className="mt-4 text-sm text-slate-300">The Closing Split regular-season schedule is saved. No re-import or regeneration is required.</p><dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4"><Metadata label="League year" value={String(state.acceptedSchedule.leagueYear)} /><Metadata label="Split" value={state.acceptedSchedule.split} /><Metadata label="Matches" value={String(state.acceptedSchedule.matches.length)} /><Metadata label="Validation" value={state.acceptedSchedule.validation.status} /></dl></details>}
    </div>;
  }
  ...
  const activate = () => {
    ...
    if (action.errors.length) return setMessage({ tone: "error", text: action.errors[0] });
    updateState(() => action.state);
    setMessage({ tone: "success", text: "Closing Split Week 1 is now the active weekly workflow." });
  };
  ...
  return <div className="space-y-8">
    ...
    {targetCompositionAccepted && <section className="border border-emerald-400/30 bg-emerald-400/10 p-6"><h2 className="text-xl font-black uppercase">Closing Split Setup Ready</h2><p className="mt-2 text-sm text-emerald-50">The reviewed post-finals league composition is accepted and ready as the Closing Split starting point.</p><dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4"><Metadata label="Global League" value={String(rosters["Global League"].length)} /><Metadata label="Continental League" value={String(rosters["Continental League"].length)} /><Metadata label="National League" value={String(rosters["National League"].length)} /><Metadata label="Regional League" value={String(rosters["Regional League"].length)} /><Metadata label="total" value={`${acceptedRosterNames.length} wrestlers`} /><Metadata label="duplicates" value={duplicateRosterNames.length ? [...new Set(duplicateRosterNames)].join(", ") : "no duplicates"} /><Metadata label="target" value={`League Year ${targetYear} · ${targetSplit} · Split Week 1 · Year Week ${yearWeekStart}`} /><Metadata label="schedule range" value={`Weeks ${yearWeekStart}–${yearWeekStart + 21}`} /></dl></section>}
    <section className="border border-white/10 bg-[#111722] p-6"><h2 className="text-xl font-black uppercase">Generate Closing Split preview</h2><p className="mt-2 text-sm text-slate-400">Target: League Year {targetYear} · {targetSplit} · Year Weeks {yearWeekStart}–{yearWeekStart + 21}. The preview becomes active only after you accept it.</p><div className="mt-4 grid gap-3 sm:grid-cols-3"><Status label="Preview status" value={preview.length ? `${preview[0].source} · ${preview.length} matches` : "No preview — generate the Closing Split schedule first."} ready={preview.length > 0} /><Status label="Validation status" value={preview.length ? validation.status : "Not run"} ready={validation.valid} /><Status label="Saved schedule" value={state.acceptedSchedule ? "Accepted and saved" : "Not accepted"} ready={Boolean(state.acceptedSchedule)} /></div><div className="mt-5 flex flex-wrap items-center gap-3"><button className="cursor-pointer bg-red-500 px-4 py-2 font-black uppercase disabled:cursor-not-allowed disabled:opacity-40" disabled={!transitionValid || !readiness.seeds.valid} onClick={generate}>Generate Closing Split Schedule</button><button className="cursor-pointer border border-white/20 px-4 py-2 font-bold disabled:cursor-not-allowed" disabled={!preview.length} onClick={exportJson}>Export JSON</button><button className="cursor-pointer border border-emerald-400/50 px-4 py-2 font-bold disabled:cursor-not-allowed disabled:opacity-40" disabled={!acceptance.enabled} onClick={accept}>Accept Schedule And Start Closing Split</button>{state.acceptedSchedule && <label className="flex cursor-pointer items-center gap-2 text-sm text-amber-200"><input type="checkbox" checked={replaceConfirmed} onChange={(event) => setReplaceConfirmed(event.target.checked)} /> Replace the existing saved schedule</label>}</div>{acceptance.disabledReason && <p className="mt-3 text-sm font-bold text-amber-200" role="status">{acceptance.disabledReason}</p>}{!targetCompositionAccepted && <><textarea aria-label="Imported schedule JSON" className="mt-5 h-36 w-full border border-white/10 bg-black/30 p-3 font-mono text-xs" placeholder="Paste generated schedule JSON" value={importText} onChange={(event) => setImportText(event.target.value)} /><button className="mt-2 cursor-pointer border border-white/20 px-4 py-2 font-bold" onClick={importJson}>Import JSON preview</button></>}</section>
    {validationAttempted && <section className={`border p-6 ${validation.valid ? "border-emerald-400/30" : "border-amber-400/30"}`}><h2 className="text-xl font-black uppercase">Validation · {preview.length ? validation.status : "No preview"}</h2><p className="mt-2 text-sm">{validation.totalMatches} / 528 matches</p>{validation.errors.length > 0 && <details className="mt-4 border border-amber-400/20 bg-amber-400/5"><summary className="cursor-pointer px-4 py-3 text-sm font-bold text-amber-200">{validation.errors.length} validation issues · Review details</summary><ul className="max-h-60 list-disc overflow-auto border-t border-amber-400/20 px-4 py-3 pl-9 text-sm text-amber-200">{validation.errors.map((error) => <li key={error}>{error}</li>)}</ul></details>}</section>}
    ...
    {state.acceptedSchedule && <details className="border border-emerald-400/30 bg-emerald-400/10 p-6"><summary className="cursor-pointer text-sm font-black uppercase text-emerald-100">Accepted schedule details</summary><dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4"><Metadata label="Accepted" value={state.acceptedSchedule.acceptedAt} /><Metadata label="League year" value={String(state.acceptedSchedule.leagueYear)} /><Metadata label="Split" value={state.acceptedSchedule.split} /><Metadata label="Matches" value={String(state.acceptedSchedule.matches.length)} /><Metadata label="Validation" value={state.acceptedSchedule.validation.status} /><Metadata label="Source" value={state.acceptedSchedule.source} />{state.acceptedSchedule.generatorVersion && <Metadata label="Generator version" value={state.acceptedSchedule.generatorVersion} />}{state.acceptedSchedule.importerVersion && <Metadata label="Importer version" value={state.acceptedSchedule.importerVersion} />}</dl><p className="mt-4 text-sm text-emerald-100">This saved schedule is ready for activation. Starting the week still requires an explicit action.</p></details>}
    {!targetCompositionAccepted && <section className="border border-red-400/30 bg-[#111722] p-6"><p className="text-xs font-black uppercase tracking-wider text-red-300">Next action</p><h2 className="mt-2 text-xl font-black uppercase">Activate Closing Split Week 1</h2><p className="mt-2 text-sm text-slate-400">Accepting the schedule does not start a week. Activation preserves all Opening Split and finals history.</p><button className="mt-5 bg-red-500 px-5 py-3 font-black uppercase disabled:cursor-not-allowed disabled:opacity-40" disabled={!activation.enabled} onClick={activate}>Activate Closing Split Week 1</button>{activation.disabledReason && <p className="mt-3 text-sm font-bold text-amber-200" role="status">Locked: {activation.disabledReason}</p>}</section>}
