...
export function ScheduleSetupView(props: Props) {
  ...
  const activate = () => {
    ...
    setMessage({ tone: "success", text: "Closing Split Week 25 is now the active browser-local weekly workflow." });
  };
  ...
  return <div className="space-y-8">
    ...
    <section className="border border-red-400/30 bg-[#111722] p-6"><h2 className="text-xl font-black uppercase">Activate weekly workflow</h2><p className="mt-2 text-sm text-slate-400">Acceptance alone does not start a week. This explicit browser-local action preserves all Opening Split and finals history.</p><button className="mt-5 bg-red-500 px-5 py-3 font-black uppercase disabled:cursor-not-allowed disabled:opacity-40" disabled={!activation.enabled} onClick={activate}>Start Closing Split / Activate Week 25</button>{activation.disabledReason && <p className="mt-3 text-sm font-bold text-amber-200" role="status">{activation.disabledReason}</p>}</section>
