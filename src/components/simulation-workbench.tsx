...
export function SimulationWorkbench({
  ...
  if (!hydrated) {
    return (
      <div className="border border-white/10 p-6 text-sm text-slate-500">
        Loading local tracker state…
      </div>
    );
  }
      ...
      <div className="flex flex-col justify-between gap-4 border border-white/10 bg-[#111722] p-5 sm:flex-row sm:items-center">
        <div>
          <p className="text-xs font-bold uppercase tracking-[.18em] text-red-400">
            Simulation Week {week}
          </p>
          <p className="mt-1 text-sm text-slate-400">
            {availableCandidates.length} unconfirmed scheduled matches across{" "}
            {leagues.length} eligible leagues. {userLeague} is excluded.
          </p>
        </div>
