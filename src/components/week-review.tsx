...
export function WeekReview({ allMatches, baselineStandings, userLeague, workbookCurrentWeek }: WeekReviewProps) {
  ...
  const progress = week === null ? null : getWeekProgress(state, week, allMatches, userLeague);
  ...
  return <div className="space-y-8">
    ...
    {progress ? <>
      ...
      <div className="flex flex-wrap gap-3 border border-white/10 bg-[#111722] p-5"><button disabled={progress.status !== "complete-unlocked"} onClick={markComplete} className="bg-emerald-500 px-4 py-3 text-xs font-black uppercase tracking-wider text-white disabled:cursor-not-allowed disabled:opacity-35">Complete & lock Week {week}</button><Link href="/results" className="border border-white/15 px-4 py-3 text-xs font-black uppercase tracking-wider text-slate-300">Result Entry</Link><Link href="/simulation" className="border border-white/15 px-4 py-3 text-xs font-black uppercase tracking-wider text-slate-300">Simulation</Link><StateControls downloadExport={downloadExport} importInput={importInput} importFile={importFile} reset={reset} /></div>
