...
2K </span>
        ...
        <span>
          <strong className="block text-sm font-black uppercase tracking-[.18em]">
            League Control
          </strong>
      ...
      <nav className="flex items-center gap-1 overflow-x-auto rounded-full border border-white/10 bg-white/5 p-1 text-sm">
        {nav.map(([label, href]) => (
          <Link
            key={href}
            href={href}
            className="whitespace-nowrap rounded-full px-3 py-2 text-slate-300 transition hover:bg-white/10 hover:text-white sm:px-4"
          >
            {label}
          </Link>
        ))}
  ...
  <footer className="mx-auto flex max-w-[1500px] justify-between border-t border-white/10 px-5 py-6 text-xs uppercase tracking-[.14em] text-slate-500 lg:px-10">
    ...
    <span>No simulated fixtures</span>
  </footer>
</div>
