...
import { detectActiveWeek, getActiveUserLeagueMatches, getWeekProgress, getWorkflowSummary } from "../week-progression";
...
describe("week progression", () => {
  ...
  it("recommends Result Entry while user-league results are missing", () => {
    const summary = getWorkflowSummary(createEmptyTrackerState(), allMatches, 13, "National League");
  ...
  it("recommends Simulation after the user show is complete", () => {
    const nationalMatches = scheduledWeek(14).filter((match) => match.league === "National League");
    const summary = getWorkflowSummary(stateWith(confirmations(nationalMatches)), allMatches, 13, "National League");
    expect(summary).toMatchObject({
    ...
    expect(summary.nonUserLeagueProgress.every((entry) => entry.missing === 6)).toBe(true);
  });

  it("recommends Week Review when all active-week results are valid", () => {
    const summary = getWorkflowSummary(stateWith(confirmations(scheduledWeek(14))), allMatches, 13, "National League");
    expect(summary).toMatchObject({
  ...
  it("reports completion without mutating tracker inputs", () => {
    ...
    expect(getWorkflowSummary(state, allMatches, 13, "National League")).toMatchObject({
      seasonComplete: true,
