...
export function buildSimulationCandidates(input: {
  ...
  for (const matchId of input.confirmedMatchIds ?? []) completedIds.add(matchId);
  const openMatches = input.matches.filter((match) => match.status === "scheduled" && !completedIds.has(match.id));
  const week = input.targetWeek === undefined
    ? (openMatches.length ? Math.min(...openMatches.map((match) => match.week)) : null)
    : input.targetWeek;
